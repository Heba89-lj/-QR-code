...
export default async function handler(req, res) {
  ...
  if (!number || !year || !nationalId) {
    return res.status(400).json({ success: false, message: "من فضلك أدخلي رقم الفحص والسنة والرقم القومي" });
  }
  ...
  try {
    ...
    // ✅ نبحث في نفس الصف على تطابق الثلاث قيم
    const match = rows.find((r) =>
      normalize(r[0]) === normalize(number) && // رقم الفحص
      normalize(r[1]) === normalize(year) &&   // السنة
      normalize(r[2]) === normalize(nationalId) // الرقم القومي
    );
    ...
    if (match) {
      ...
      return res.status(404).json({
        success: false,
        message: "لم يتم العثور على بيانات تطابق رقم الفحص والسنة والرقم القومي",
      });
    }
  } catch (error) {
    console.error("🔥 Error fetching Google Sheet:", error);
